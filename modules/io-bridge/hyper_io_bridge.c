...
static void notify_call(struct call_binding *call)
{
	 ...
	 * no-lock drain is exported for exactly this use; eventfd_ctx_read
	 * would recursively acquire the same lock and deadlock. */
	eventfd_ctx_do_read(call->event, &count);
	if (count)
		writeq(((u64)call->epoch << 32) | 1, call->bridge->base + 0x18);
}
static int call_wakeup(wait_queue_entry_t *wait, unsigned int mode, int sync,
		       void *key)
{
	struct call_binding *call =
	    container_of(wait, struct call_binding, wait);
	if (key_to_poll(key) & EPOLLIN)
		notify_call(call);
	return 0;
}
static void call_poll_queue(struct file *file, wait_queue_head_t *queue,
			    poll_table *table)
{
	struct call_binding *call =
	    container_of(table, struct call_poll, table)->call;
...
static void unbind(struct bridge *bridge)
{
	 ...
	 * excludes all readers of kick[] before eventfd references are
	 * released. */
	...
	for (i = 0; i < HYPER_IO_QUEUES; ++i) {
		...
		if (call->queue) {
			u64 count;
			eventfd_ctx_remove_wait_queue(call->event, &call->wait,
						      &count);
			call->queue = NULL;
		}
		if (call->event) {
			eventfd_ctx_put(call->event);
			call->event = NULL;
		}
		if (call->file) {
			fput(call->file);
		...
		if (bridge->kick[i]) {
			eventfd_ctx_put(bridge->kick[i]);
			bridge->kick[i] = NULL;
		}
	}
	bridge->bound = false;
}
static long notification_ioctl(struct file *file, unsigned int command,
			       unsigned long argument)
{
	...
	if (bridge->mailbox)
		return -ENOTTY;
	if (command != HYPER_IO_BIND && command != HYPER_IO_UNBIND &&
	    command != HYPER_IO_QUIESCENT && command != HYPER_IO_ADMIT &&
	    command != HYPER_IO_EXTENT)
		return -ENOTTY;
	...
	if (command == HYPER_IO_ADMIT) {
		...
		if (copy_from_user(&grant, (void __user *)argument,
				   sizeof(grant))) {
			result = -EFAULT;
			goto out;
		}
		...
		/* Admission atomically binds the one-use token to this route.
		 * Query immutable kernel facts instead of trusting a claimed
		 * alias/length. */
		writeq(grant.token, bridge->base + 0x28);
		if (readq(bridge->base + 0x40)) {
			result = -EACCES;
			goto out;
		}
		if (readq(bridge->base + 0x30) != grant.guest_base ||
		    readq(bridge->base + 0x38) != grant.length) {
			/* Admitted but never touched: retire this token before
			 * rejection. */
		...
		if (!grant.extent_count ||
		    grant.extent_count > grant.length / 4096 ||
		    copy_to_user((void __user *)argument, &grant,
				 sizeof(grant))) {
			writeq(grant.token, bridge->base + 0x20);
			result = -EINVAL;
		}
	...
	if (command == HYPER_IO_EXTENT) {
		...
		if (copy_from_user(&extent, (void __user *)argument,
				   sizeof(extent))) {
			result = -EFAULT;
			goto out;
		}
		writeq(extent.index, bridge->base + 0x50);
		if (readq(bridge->base + 0x70)) {
			result = -EINVAL;
			goto out;
		}
		...
		if (copy_to_user((void __user *)argument, &extent,
				 sizeof(extent)))
			result = -EFAULT;
		goto out;
	}
	if (command == HYPER_IO_QUIESCENT) {
		...
		if (copy_from_user(&token, (void __user *)argument,
				   sizeof(token))) {
			result = -EFAULT;
			goto out;
		}
		...
		/* Hyper derives the sender VM from the trapped write, never
		 * from a caller-controlled Native capability or a claimed peer
		 * identifier. */
		writeq(token, bridge->base + 0x20);
		goto out;
	}
	if (command == HYPER_IO_UNBIND) {
		unbind(bridge);
		goto out;
	}
	if (bridge->bound) {
	...
	if (copy_from_user(&fds, (void __user *)argument, sizeof(fds))) {
		result = -EFAULT;
		goto out;
	}
	...
	/* Unbound notification IRQs remain masked, including before a route
	 * exists. */
	for (i = 0; i < HYPER_IO_QUEUES; ++i) {
		struct call_binding *call = &bridge->call[i];
		...
		if (IS_ERR(bridge->kick[i])) {
			result = PTR_ERR(bridge->kick[i]);
			bridge->kick[i] = NULL;
			break;
		}
		call->file = eventfd_fget(fds.call[i]);
		if (IS_ERR(call->file)) {
			result = PTR_ERR(call->file);
			call->file = NULL;
			break;
		}
		call->event = eventfd_ctx_fileget(call->file);
		if (IS_ERR(call->event)) {
			result = PTR_ERR(call->event);
			call->event = NULL;
			break;
		}
		call->bridge = bridge;
		call->epoch = fds.epoch;
...
static irqreturn_t bridge_irq(int irq, void *opaque)
{
	...
	if (bridge->mailbox) {
		/* One-shot source arming. Readers/writers recheck durable
		 * status before sleeping and restore their desired mask. */
...
{
	struct bridge *bridge =
	    container_of(file->private_data, struct bridge, misc);
	if (atomic_cmpxchg(&bridge->opened, 0, 1))
		return -EBUSY;
...
static int wait_status(struct bridge *bridge, u32 wanted)
{
	...
	for (;;) {
		...
		if (status & wanted)
			return 0;
		/* A delayed IRQ from a previous transaction may mask this new
		 * arm. Waking on its generation change forces the outer loop to
		 * rearm, even if the new transaction has no RX data yet. */
		...
		result = wait_event_interruptible(
			...
			hyper_io_wait_ready(
			    readl(bridge->base + MB_STATUS), wanted, generation,
			    atomic64_read(&bridge->irq_generation)));
		writel(0, bridge->base + MB_MASK);
		if (result)
			return result;
	}
}
static ssize_t control_read(struct file *file, char __user *buffer,
			    size_t capacity, loff_t *offset)
{
	struct bridge *bridge = file->private_data;
	u32 bytes[64], length;
	u64 sequence;
	...
	if (!bridge->mailbox)
		return -EINVAL;
	if (mutex_lock_interruptible(&bridge->control_lock))
		return -ERESTARTSYS;
	...
	for (i = 0; i < DIV_ROUND_UP(length, 4); ++i)
		bytes[i] =
		    cpu_to_le32(readl(bridge->base + MB_RX_DATA + i * 4));
	if (copy_to_user(buffer, bytes, length)) {
		result = -EFAULT;
		goto out;
	}
	/* Retain the record on user-copy failure; consume only its exact
	 * sequence. */
			     ...
			     size_t length, loff_t *offset)
{
	...
	ssize_t result;
	unsigned int i;
	if (!bridge->mailbox || !length || length > sizeof(bytes))
		return -EINVAL;
	if (copy_from_user(bytes, buffer, length))
		return -EFAULT;
	if (mutex_lock_interruptible(&bridge->control_lock))
		return -ERESTARTSYS;
	...
	for (i = 0; i < DIV_ROUND_UP(length, 4); ++i)
		writel(le32_to_cpu(bytes[i]),
		       bridge->base + MB_TX_DATA + i * 4);
	writel(length, bridge->base + MB_TX_LEN);
	writel(1, bridge->base + MB_TX_COMMIT);
	result =
	    readl(bridge->base + MB_STATUS) & (ERROR | CLOSED) ? -EIO : length;
out:
	mutex_unlock(&bridge->control_lock);
	return result;
}
static const struct file_operations bridge_operations = {
    ...
    .read = control_read,
    .write = control_write,
    .unlocked_ioctl = notification_ioctl,
...
static int bridge_probe(struct platform_device *device)
{
	...
	bridge->mailbox = of_device_is_compatible(device->dev.of_node,
						  "hyper,guest-mailbox-v1");
	resource = platform_get_resource(device, IORESOURCE_MEM, 0);
	if (!resource || resource_size(resource) < 4096) {
		result = -EINVAL;
		goto fail;
	}
	bridge->base = devm_ioremap_resource(&device->dev, resource);
	if (IS_ERR(bridge->base)) {
		result = PTR_ERR(bridge->base);
		goto fail;
	}
	/* Managed notification routes are installed only when a client binds.
	 */
	if ((bridge->mailbox ||
	     !of_find_property(device->dev.of_node, "hyper,client-id", NULL)) &&
	    (readl(bridge->base) !=
		 (bridge->mailbox ? 0x48594d42 : 0x48594e42) ||
	     readl(bridge->base + 4) != 1)) {
		result = -ENODEV;
		goto fail;
	}
	mutex_init(&bridge->control_lock);
	mutex_init(&bridge->bind_lock);
	init_waitqueue_head(&bridge->changed);
	atomic_set(&bridge->opened, 0);
	...
	/* Threaded IRQ context avoids nested eventfd wake recursion and lets
	 * the supported vhost eventfd consumer schedule its normal kernel
	 * worker. */
	result = devm_request_threaded_irq(
	    &device->dev, bridge->irq, NULL, bridge_irq,
	    IRQF_ONESHOT | (bridge->mailbox ? 0 : IRQF_NO_AUTOEN),
	    dev_name(&device->dev), bridge);
	if (result)
		goto fail;
	bridge->misc.minor = MISC_DYNAMIC_MINOR;
	{
		...
		if (of_find_property(device->dev.of_node, "hyper,client-id",
				     NULL)) {
			if (of_property_read_u32(device->dev.of_node,
						 "hyper,client-id", &client) ||
			    client >= 128) {
				result = -EINVAL;
				goto free_irq;
			}
	...
	bridge->misc.mode = 0600;
	bridge->misc.fops = &bridge_operations;
...
static void bridge_remove(struct platform_device *device)
{
	 ...
	 * before devm releases IRQ/MMIO. Open files retain only the dead
	 * object. */
...
static const struct of_device_id bridge_match[] = {
    {.compatible = "hyper,guest-mailbox-v1"},
    {.compatible = "hyper,guest-notification-v1"},
    {}};
MODULE_DEVICE_TABLE(of, bridge_match);
static struct platform_driver bridge_driver = {
    .probe = bridge_probe,
    .remove = bridge_remove,
    .driver = {.name = "hyper-io-bridge",
	       .of_match_table = bridge_match,
