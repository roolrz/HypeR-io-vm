...
{
	/* CLEAR_ENDPOINT clears all queue backends and flushes in-flight
	 * commands. Never replace this with KICK=-1 or GET_VRING_BASE: neither
	 * drains SCSI. */
	if (b->attached) {
	...
	/* All vhost producers are gone before callbacks and their epoch
	 * disappear. */
	if (b->bound) {
...
static int release_memory(struct backend *b)
{
	 ...
	 * mapping. */
	if (b->token && ioctl(b->notification, HYPER_IO_QUIESCENT, &b->token))
		return HYPER_IO_QUIESCENCE_FAILED;
	b->token = 0;
	memset(&b->region, 0, sizeof(b->region));
	return HYPER_IO_OK;
}
static int prepare_memory(struct backend *b,
			  const struct hyper_io_prepare_memory *request)
{
	...
	if (prepare.token) {
		...
		if (prepare.alias || length / 4096 > HYPER_IO_MAX_GRANT_PAGES)
			return HYPER_IO_INVALID;
		if (ioctl(b->notification, HYPER_IO_ADMIT, &admission))
			return HYPER_IO_INVALID;
...
static int activate(struct backend *b, const struct hyper_io_activate *message,
		    uint64_t offered)
{
	...
	struct hyper_io_eventfds bridge = {.epoch =
					       le64toh(message->header.epoch),
					   .kick = {-1, -1, -1, -1, -1, -1},
					   .call = {-1, -1, -1, -1, -1, -1}};
	if (b->fd >= 0 || b->bound || b->attached)
		return HYPER_IO_BUSY;
	...
	diagnostic(b, "configuring vhost-scsi: epoch=%llu",
		   (unsigned long long)bridge.epoch);
	b->fd = open("/dev/vhost-scsi", O_RDWR | O_CLOEXEC);
	if (b->fd < 0 || ioctl(b->fd, VHOST_SET_OWNER, NULL) ||
	    ioctl(b->fd, VHOST_SET_FEATURES, &features) ||
	    ioctl(b->fd, VHOST_SET_MEM_TABLE, &table))
		goto fail;
	for (unsigned int i = 0; i < HYPER_IO_QUEUES; ++i) {
		if (!message->queues[i].size)
			continue;
		struct vhost_vring_state state = {
		    .index = i, .num = le32toh(message->queues[i].size)};
		...
		if (ioctl(b->fd, VHOST_SET_VRING_NUM, &state) ||
		    ioctl(b->fd, VHOST_SET_VRING_ADDR, &address))
			goto fail;
		state.num = 0;
		if (ioctl(b->fd, VHOST_SET_VRING_BASE, &state))
			goto fail;
		b->kick[i] = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK);
		b->call[i] = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK);
		if (b->kick[i] < 0 || b->call[i] < 0)
			goto fail;
		event.fd = b->kick[i];
		if (ioctl(b->fd, VHOST_SET_VRING_KICK, &event))
			goto fail;
		event.fd = b->call[i];
		if (ioctl(b->fd, VHOST_SET_VRING_CALL, &event))
			goto fail;
		bridge.kick[i] = b->kick[i];
		bridge.call[i] = b->call[i];
	}
	diagnostic(b, "binding notifications: epoch=%llu",
		   (unsigned long long)bridge.epoch);
	if (ioctl(b->notification, HYPER_IO_BIND, &bridge))
		goto fail;
	b->bound = 1;
	diagnostic(b, "attaching vhost-scsi endpoint");
	if (ioctl(b->fd, VHOST_SCSI_SET_ENDPOINT, &b->target))
		goto fail;
	b->attached = 1;
	return HYPER_IO_OK;
fail:
	fprintf(stderr, "HypeR I/O: activation failed: %s\n", strerror(errno));
	return quiesce(b) ? HYPER_IO_QUIESCENCE_FAILED
			  : HYPER_IO_BACKEND_FAILURE;
}
static int serve(struct backend *b, int control, uint64_t features)
{
	...
	for (;;) {
		...
		if (stopping)
			return 0;
		/* Peer close is administrative shutdown; main still drains
		 * before exit. */
		if (size < 0 && errno == EPIPE)
			return 0;
		if (size <= 0)
			return -1;
		...
		uint64_t request_binding = le64toh(header.binding),
			 transaction = le64toh(header.transaction);
		if (!le64toh(header.epoch))
			return -1;
		if (le32toh(header.magic) != HYPER_IO_MAGIC ||
		    ...
		    !request_binding || !transaction ||
		    le64toh(header.epoch) > UINT32_MAX)
			return -1;
		uint16_t operation = le16toh(header.operation);
		if (operation < HYPER_IO_HELLO ||
		    operation > HYPER_IO_RELEASE_MEMORY)
			diagnostic(b, "request UNKNOWN (opcode=%u)",
				   (unsigned)operation);
		else
			diagnostic(b, "request %s", operation_name(operation));
		int admission = hyper_io_session_admit(
		    &session, request_binding, le64toh(header.epoch),
		    operation == HYPER_IO_HELLO && size == 40,
		    b->managed ? b->memory == MAP_FAILED && b->memory_fd < 0 &&
				     b->fd < 0 && !b->bound && !b->token
			       : !session.binding);
		if (admission > 0) {
			last_transaction = 0;
			previous_size = 0;
		...
		if (admission >= 0 && transaction == last_transaction &&
			...
			/* Same transaction is replay-only, including failures.
			 * Retrying a failed drain requires a fresh transaction
			 * and RESET. */
			...
			if (admission >= 0 && transaction > last_transaction) {
				if (operation == HYPER_IO_HELLO && size == 40 &&
				    hyper_io_session_preparable(&session)) {
					status = HYPER_IO_OK;
					reply_size = 64;
					response.features = htole64(features);
					response.queues =
					    htole32(HYPER_IO_QUEUES);
					response.queue_max = htole32(128);
				} else if (operation ==
					       HYPER_IO_PREPARE_MEMORY &&
					   size == 72 && b->managed &&
					   hyper_io_session_preparable(
					       &session)) {
					struct hyper_io_prepare_memory request;
					memcpy(&request, message,
					       sizeof(request));
					...
					if (status == HYPER_IO_OK)
						hyper_io_session_retire(
						    &session);
				} else if (operation == HYPER_IO_ACTIVATE &&
					   size == 240) {
					struct hyper_io_activate request;
					memcpy(&request, message,
					       sizeof(request));
					status =
					    activate(b, &request, features);
				} else if (((operation == HYPER_IO_RESET &&
					     size == 40) ||
					    (operation == HYPER_IO_STOP_QUEUE &&
					     size == 48))) {
					uint32_t queue = 0, reserved = 0;
					if (size == 48) {
						memcpy(&queue, message + 40, 4);
						memcpy(&reserved, message + 44,
						       4);
					}
					if (le32toh(queue) < HYPER_IO_QUEUES &&
					    !reserved)
						status =
						    quiesce(b)
							? HYPER_IO_QUIESCENCE_FAILED
			...
			response.header.length = htole32(reply_size);
			response.header.flags = htole32(HYPER_IO_REPLY);
			response.status = htole32(status);
			if (admission >= 0 && transaction > last_transaction) {
				if (le64toh(header.epoch) > session.epoch)
					session.epoch = le64toh(header.epoch);
		...
		if (le32toh(response.status) == HYPER_IO_OK)
			diagnostic(b, "reply %s: ok",
				   operation_name(operation));
		else
			diagnostic(b, "reply %s: %s (status=%u)",
				   operation_name(operation),
				   status_name(le32toh(response.status)),
				   le32toh(response.status));
		ssize_t written;
		do {
			written = write(control, &response,
					le32toh(response.header.length));
		...
		if (written != (ssize_t)le32toh(response.header.length))
			diagnostic(b,
				   "reply write incomplete: written=%zd, "
				   "expected=%u bytes",
...
{
	struct backend b = {.notification = -1,
			    .memory_fd = -1,
			    .fd = -1,
			    .kick = {-1, -1, -1, -1, -1, -1},
			    .call = {-1, -1, -1, -1, -1, -1},
	...
	if (!standby && !b.managed) {
		...
		b.notification =
		    open("/dev/hyper-io-notification", O_RDWR | O_CLOEXEC);
		if (b.memory == MAP_FAILED || b.notification < 0)
			return 1;
	}
	if (b.managed)
		b.notification = open(argv[4], O_RDWR | O_CLOEXEC);
	if (b.managed && b.notification < 0)
		return 1;
	...
	if (control < 0 || probe < 0 ||
	    ioctl(probe, VHOST_GET_FEATURES, &features) ||
	    !(features & VERSION_1))
		return 1;
	close(probe);
	features &= VERSION_1;
	printf("HypeR I/O: %s service ready [%s]\n",
	       standby ? "standby" : "backend", b.target.vhost_wwpn);
	fflush(stdout);
	int result = serve(&b, control, features);
	if (release_memory(&b) != HYPER_IO_OK) {
		/* No successful acknowledgement or unmap is allowed after
		 * failed drain. Keep the process and all resources alive for
	...
	close(control);
	if (b.notification >= 0)
		close(b.notification);
	if (b.memory != MAP_FAILED)
		munmap(b.memory, b.region.length);
	if (b.memory_fd >= 0)
		close(b.memory_fd);
	return result ? 1 : 0;
}
